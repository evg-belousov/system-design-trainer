      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Системный Анализ</h2>
            <p className="text-gray-500 text-sm mb-4">{saCount} вопросов</p>
            <ul className="space-y-2">
              {saTopics.map(t => (
                <li key={t.id} className="text-gray-700 text-sm">• {t.label}</li>
              ))}
            </ul>
          </div>

          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 mb-2">System Design</h2>
            <p className="text-gray-500 text-sm mb-4">{sdCount} вопросов</p>
            <ul className="space-y-2">
              {sdTopics.map(t => (
                <li key={t.id} className="text-gray-700 text-sm">• {t.label}</li>
              ))}
